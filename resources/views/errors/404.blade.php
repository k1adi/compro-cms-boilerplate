    ...
    <main class="section__wrapper !h-dvh grid place-items-center">
        <div class="flex flex-col md:flex-row items-center gap-8 px-5">
            <div class="w-full md:full-1/2 order-2 md:order-1">
                <h1 class="text-4xl mb-2 font-semibold">{{ __('page.404-heading') }}</h1>
                <p class="text-gray-500">{{ __('page.404-text') }}</p>
                <a href="{{ route('home', ['locale' => app()->getLocale()]) }}" class="text-primary font-semibold block mt-4">{{ __('page.404-link') }}</a>
            </div>
            <div class="w-full md:full-1/2 order-1 md:order-2">
                <img src="{{ asset('img/not-found.svg') }}" class="w-full max-w-[400px]" alt="Not found illustration">
            </div>
        </div>
    </main>
</body>
